...
VIDEO_PATH = Path(__file__).parent / "assets" / "tiger.mp4"
   ...
   const mobileMenuButton = document.getElementById('mobile-menu-button');
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        generatedPromptDiv.textContent = result.prompt;
        resultSection.classList.remove('hidden');
      } catch (err) {
        errorMessageSpan.textContent = 'Network error or server unavailable.';
        errorSection.classList.remove('hidden');
      }
    });

    copyButton.addEventListener('click', () => {
      if (generatedPromptDiv.textContent) {
        navigator.clipboard.writeText(generatedPromptDiv.textContent).then(() => {
...
<head>
    ...
    <style>
        ...
        .video-container { background: #f0f0f0; padding: 20px; border-radius: 8px; margin-top: 20px; }
        .download-btn { 
...
<body>
    ...
    <div class="video-container">
        ...
        <a href="/video" download class="download-btn">Download Video</a>
    </div>
        <p style="margin-top: 20px; color: #666;">
        Server status: ✅ Online | Video available: {'✅' if VIDEO_PATH.exists() else '❌ Missing'}<br>
        API Key: {'✅ Present' if os.getenv('POE_API_KEY') else '❌ Missing'} | Path: {str(VIDEO_PATH.absolute())}
        </p>
